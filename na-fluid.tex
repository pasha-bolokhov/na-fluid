...
\marginpar{\vspace{1.0cm} \tiny This paragraph needs to be re-phrased}
...
\subsection{Tree-level energy momentum tensor}
	We now again assume that the Lagrangian is a generic function of $ n^2 $ and $ \varphi^2 $,
\beq
	\mc{L}    ~~=~~    -\, f( n^2, \varphi^2 )\,.
\eeq
	In the Abelian string-fluid plasma, the field strength $ F^{\mu\nu} $ happens to be a bivector,
...
\beq
	...
	( q_\mu^\slashed a )^2    ~~=~~    1\,.
\eeq
	Even though vectors $ q_\mu^a $ are not suppressed by the coupling constant (they have a unitary length),
        the only source they appear from
        in the energy-momentum tensor is field strength \eqref{G-pert}, via the commutator, which \emph{is} suppressed.
	So the expansion in coupling constant $ g $ will correspond to an expansion in (even) powers of $ q_\mu^a $.
        
	Utilizing expression \eqref{G-pert} we find
\begin{align}
%
\notag
...
\notag
	T_\text{fluid}^{\mu\nu}    &
					~~=~~
						\bigl\lgroup 2\, f_{n^2}^\prime \bigg|_{\varphi_{(0)}} (n^a)^2  ~+~
...
\end{align}
	...
	in terms of $ u^{\mu a} $.
        ...
        We now briefly discuss the large-$ N $ limit of energy-momentum tensor \eqref{Ttotal}.
        The vector structure of the tensor gets simplified.
        Since velocity vectors $ u^{a \mu} $ are time-like, they will dominantly will add up
        as we are summing in index $ a $.
        In other words, in the absence of cross-correlations, by averaging in $ a $, 
        we are extracting a time-like component from all of $ u^{a \mu} $,
\beq
	\langle u^{a \mu} \rangle    ~~=~~    U^\mu\,.
\eeq
	On the other hand, vectors $ w^{a \mu} $ are space-like, and therefore, when summed up in $ a $,
        they will just perform a random walk.
        Even though the length of a random walk is $ \sqrt{N} $,
        the average of individual components of a vector is zero.
        Taking into account that
\begin{align}
	\langle (n^a)^2 \rangle    & ~~=~~    \frac{1}{N}\,n^2\,,
        &
        \langle (\varphi^a)^2 \rangle     & ~~=~~    \frac{1}{N}\, (\phi_{(0)})^2\,,
\end{align}
        the tree-level energy-momentum tensor at large $ N $ therefore has the following structure
\begin{align}
%
\notag
	T_\text{large-$ N $}^{\mu\nu}    &
...
\notag
	& 
					...
					O\big(\, q^2 \,\big)\,.
\end{align}
	This is a single-component fluid with an explicit expression for pressure and with energy density equal to $ f $.
